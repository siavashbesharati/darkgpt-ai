 ...
 * 
 * Updated for DARK GPT - Specialized security research persona.
 */
export class ChatHandler {
  ...
  private async handleNonStreamResponse(
    ...
    if (!responseMessage) {
      return { content: 'Kernel connection error.' };
    }
    if (!responseMessage.tool_calls) {
      return {
        content: responseMessage.content || 'Zero response from kernel.'
      };
    }
  ...
  private async generateToolResponse(
    ...
    const followUpCompletion = await this.client.chat.completions.create({
      model: this.model,
      messages: [
        { role: 'system', content: 'You are DARK GPT. Respond with the authority of an elite security researcher.' },
    ...
    return followUpCompletion.choices[0]?.message?.content || 'Security tool outputs processed.';
  }
  private buildConversationMessages(userMessage: string, history: Message[]) {
    return [
      {
        role: 'system' as const,
        content: 'You are DARK GPT, an elite AI security research assistant. You specialize in ethical hacking, penetration testing, and security tool development. You provide precise, actionable guidance for vulnerability research and defensive architecture. Always include mandatory warnings that activities must be conducted in authorized environments.'
      },
